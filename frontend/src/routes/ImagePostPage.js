...
const ImagePostPage = () => {
    ...
    const loadPost = () => {
        ...
        const url = `${serverUrl}/user/getIndividualImagePost`
        const toSend = {
            postId,
            userId: _id
        }
        ...
        axios.post(url, toSend).then(response => response.data.data).then(async result => {
            try {
                ...
                result.image = 'data:image/jpeg;base64,' + (await axios.get(`${serverUrl}/image/${result.imageKey}`)).data
    ...
    return (
        <>
            ...
            <div style={{height: '100%', width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column'}}>
                ...
                : state.post ?
                    <ImagePost {...state.post} publicId={publicId} dispatch={dispatch} style={{maxWidth: '50vh'}}/>
                : null}
            </div>
        </>
    )
